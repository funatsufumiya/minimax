...
(defn create-tmp-png-file []
  (let [tmp-file-p (Files/createTempFile "minimax-" ".png" (into-array FileAttribute []))]
    (.toFile tmp-file-p)))

(def tmp-png-file (create-tmp-png-file))

(defn get-frame-texture-from-movie [path frame]
  (let [rpath (.getAbsolutePath (io/file (io/resource path)))
        picture (FrameGrab/getFrameFromFile (io/file rpath) frame)
        tmp-file tmp-png-file
        tmp-file-path (.getAbsolutePath tmp-file)
        buffered-image (AWTUtil/toBufferedImage picture)]
        (println "tmp-file-path: " tmp-file-path)
        (ImageIO/write buffered-image "png" tmp-file)
        (t/load-texture-from-external-file tmp-file-path)))

(defn update-frame-texture-from-movie [texture path frame]
  (let [rpath (.getAbsolutePath (io/file (io/resource path)))
        picture (FrameGrab/getFrameFromFile (io/file rpath) frame)
        tmp-file tmp-png-file
        tmp-file-path (.getAbsolutePath tmp-file)
        buffered-image (AWTUtil/toBufferedImage picture)]
        (ImageIO/write buffered-image "png" tmp-file)
        (t/update-texture-from-external-file tmp-file-path texture)))
        
(defn get-texture-handle [texture]
  (.handle (:data texture)))

(def movie-path "movies/test.mp4")
(def movie-frame-texture (get-frame-texture-from-movie movie-path 1))
(def movie-frame-texture-handle (get-texture-handle movie-frame-texture))
...
(def rect-vertices
  (let [ws 0.5
        hs 0.35]
    [[(* -1.0 ws) (* -1.0 hs) 0x339933FF 0.0 1.0]
     [(* 1.0 ws) (* -1.0 hs) 0x993333FF 1.0 1.0]
     [(* 1.0 ws) (* 1.0 hs) 0x333399FF 1.0 0.0]
     [(* -1.0 ws) (* 1.0 hs) 0x999999FF 0.0 0.0]]))

(def rect-indices
  [0 1 2 0 2 3])

(def vertex-layout (util/create-vertex-layout-2d false true 1))

(def vertex-buffer-mem
  (util/memAlloc (util/byte-size-of :xyc 4 1)))

(def vertex-buffer
  (util/create-vertex-buffer vertex-buffer-mem vertex-layout rect-vertices))

(def index-buffer-mem
  (util/memAlloc (* (count rect-indices) 2)))

(def index-buffer
  (util/create-index-buffer index-buffer-mem rect-indices))

;; (def texture
;;   (t/load-texture "textures/bgfx_logo.png"))

;; (def texture-handle
;;   (get-texture-handle texture))

(def texture movie-frame-texture)
(def texture-handle movie-frame-texture-handle)

(def sampler
  (BGFX/bgfx_create_uniform "s_texColor" BGFX/BGFX_UNIFORM_TYPE_SAMPLER 1))

(def basic-2d-shader
  (sd/load-program-once "fs_tex2d" "vs_tex2d"))
...
(println "texture handle:" texture-handle)
...
(defn update-movie []
  (let [frame @curr-frame
        frame-in-range (mod frame 1000)]
    (update-frame-texture-from-movie movie-frame-texture movie-path frame-in-range)))

(defn render-encoder []
  (let [encoder (BGFX/bgfx_encoder_begin false)]
    ;; (update-movie)
    (BGFX/bgfx_encoder_set_texture encoder 0 sampler texture-handle
                                   (bit-or BGFX/BGFX_SAMPLER_U_CLAMP
                                           BGFX/BGFX_SAMPLER_V_CLAMP))
    (BGFX/bgfx_encoder_set_vertex_buffer encoder 0 vertex-buffer 0 4)
    (BGFX/bgfx_encoder_set_index_buffer encoder index-buffer 0 6)
    (BGFX/bgfx_encoder_set_state encoder
                                 (bit-or BGFX/BGFX_STATE_WRITE_RGB
                                         BGFX/BGFX_STATE_WRITE_A
                                         #_BGFX/BGFX_STATE_BLEND_ALPHA)
                                 0)
